#ifndef kernel_HC_Solver_cyclic9_cu
#define kernel_HC_Solver_cyclic9_cu
// ============================================================================
// homotopy continuation solver for cyclic9 problem
//
// Modifications
//    Chien  21-12-31:   cyclic9 benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-eval/dev-eval-indxing-cyclic9.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_cyclic9(
    magma_int_t ldda,
    ...
    )
  {
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_cdt_cyclic9<N, coefsCount>( tx, t0, s_cdt, s_startCoefs, s_targetCoefs );
        eval_Jacobian_cyclic9_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
        eval_Ht_cyclic9_cached<N, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>( tx, s_track, r_cgesvB, d_const_vec_cd, d_Ht_idx);
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          eval_Jacobian_cyclic9_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>(tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
          eval_H_cyclic9_cached<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>(tx, s_track, r_cgesvB, s_cdt, d_Ht_idx);
  ...
  )
  {
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 9*8*10=720 --
    // -- Ht_max_terms*Ht_max_parts = 9*11 = 99 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_cyclic9< 9, 2, 24, 5, 10, 720, 99, 8, 10, 9, 11>, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_cyclic9 is not successful!\n");
    }
