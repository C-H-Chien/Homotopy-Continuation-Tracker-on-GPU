#ifndef kernel_HC_Solver_katsura14_cu
#define kernel_HC_Solver_katsura14_cu
// ============================================================================
// homotopy continuation solver for katsura14 problem
//
// Modifications
//    Chien  22-01-24:   katsura14 benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-eval/dev-eval-indxing-katsura14.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_katsura14(
    magma_int_t ldda,
    ...
    )
  {
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_cdt_katsura14<N, coefsCount>( tx, t0, s_cdt, s_startCoefs, s_targetCoefs );
        eval_Jacobian_katsura14_improve<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts>(s_track, r_cgesvA, s_cdt, r_Hx_idx);
        eval_Ht_katsura14_improve<N, size_Ht, Ht_max_terms, Ht_max_parts>(s_track, r_cgesvB, d_const_vec_cd, r_Ht_idx);
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          eval_Jacobian_katsura14_improve<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts>(s_track, r_cgesvA, s_cdt, r_Hx_idx);
          eval_H_katsura14_improve<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts>(s_track, r_cgesvB, s_cdt, r_Ht_idx);
  ...
  )
  {
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 15*3*3=135 --
    // -- Ht_max_terms*Ht_max_parts = 16*4 = 64 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_katsura14< 15, 3, 64, 5, 10, 135, 64, 3, 3, 16, 4>, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_katsura14 is not successful!\n");
    }
