#ifndef kernel_HC_Solver_katsura21_cu
#define kernel_HC_Solver_katsura21_cu
// ============================================================================
// homotopy continuation solver for katsura21 problem
//
// Modifications
//    Chien  22-01-29:   katsura21 benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-eval/dev-eval-indxing-katsura21.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_katsura21(
    magma_int_t ldda,
    ...
    )
  {
    ...
    // ===================== test ===================
    /*magma_int_t* d_Hx_idx = d_Hx_idx_array[0];
    magma_int_t* d_Ht_idx = d_Ht_idx_array[0];
    magma_int_t r_Hx_idx[size_Hx] = {0};
    magma_int_t r_Ht_idx[size_Ht] = {0};
    #pragma unroll
    for(int i = 0; i < size_Hx; i++) {
      r_Hx_idx[i] = d_Hx_idx[ tx*size_Hx + i ];
    }
    #pragma unroll
    for(int i = 0; i < size_Ht; i++) {
      r_Ht_idx[i] = d_Ht_idx[ tx*size_Ht + i ];
    }*/
    // ==============================================
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_cdt_katsura21<N, coefsCount>( tx, t0, s_cdt, s_startCoefs, s_targetCoefs );
        //eval_Jacobian_katsura21_improve<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts>(s_track, r_cgesvA, s_cdt, r_Hx_idx);
        //eval_Ht_katsura21_improve<N, size_Ht, Ht_max_terms, Ht_max_parts>(s_track, r_cgesvB, d_const_vec_cd, r_Ht_idx);
        eval_Jacobian_katsura21_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
        eval_Ht_katsura21_cached<N, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>( tx, s_track, r_cgesvB, d_const_vec_cd, d_Ht_idx);
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          //eval_Jacobian_katsura21_improve<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts>(s_track, r_cgesvA, s_cdt, r_Hx_idx);
          //eval_H_katsura21_improve<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts>(s_track, r_cgesvB, s_cdt, r_Ht_idx);
          eval_Jacobian_katsura21_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>(tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
          eval_H_katsura21_cached<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>(tx, s_track, r_cgesvB, s_cdt, d_Ht_idx);
  ...
  )
  {
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 22*3*3=189 --
    // -- Ht_max_terms*Ht_max_parts = 23*4 = 92 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_katsura21< 22, 3, 217, 5, 10, 198, 92, 3, 3, 23, 4>, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_katsura21 is not successful!\n");
    }
