#ifndef kernel_HC_Solver_katsura8_cu
#define kernel_HC_Solver_katsura8_cu
// ============================================================================
// homotopy continuation solver for katsura8 problem
//
// Modifications
//    Chien  21-12-16:   katsura8 benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-eval/dev-eval-indxing-katsura8.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_katsura8(
    magma_int_t ldda,
    ...
    )
  {
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_cdt_katsura8<N, coefsCount>( tx, t0, s_cdt, s_startCoefs, s_targetCoefs );
        eval_Jacobian_katsura8_improve<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts>(s_track, r_cgesvA, s_cdt, r_Hx_idx);
        eval_Ht_katsura8_improve<N, size_Ht, Ht_max_terms, Ht_max_parts>(s_track, r_cgesvB, d_const_vec_cd, r_Ht_idx);
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          eval_Jacobian_katsura8_improve<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts>(s_track, r_cgesvA, s_cdt, r_Hx_idx);
          eval_H_katsura8_improve<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts>(s_track, r_cgesvB, s_cdt, r_Ht_idx);
  ...
  )
  {
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 9*3*3=81 --
    // -- Ht_max_terms*Ht_max_parts = 10*4 = 40 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_katsura8< 9, 3, 24, 5, 10, 81, 40, 3, 3, 10, 4>, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_katsura8 is not successful!\n");
    }
