#ifndef kernel_HC_Solver_3view_unknownf_pHC_cu
#define kernel_HC_Solver_3view_unknownf_pHC_cu
// ============================================================================
// GPU homotopy continuation solver for 3view_unknownf_pHC problem
//
// Modifications
//    Chien  21-12-13:   3view_unknownf_pHC benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-evals/dev-eval-indxing-3view_unknownf_pHC.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_3view_unknownf_pHC(
    magma_int_t ldda,
  ...
  )
  {
    ...
    const magmaFloatComplex* __restrict__ d_const_phc_coeffs_H = d_phc_coeffs_H;
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_parameter_homotopy<N, 54, 36>( tx, t0, s_phc_coeffs_Hx, s_phc_coeffs_Ht, d_const_phc_coeffs_H, d_const_phc_coeffs_Ht );
        eval_Jacobian_3view_unknownf_pHC< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
        eval_Ht_3view_unknownf_pHC< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Ht );
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          ...
          eval_parameter_homotopy<N, 54, 36>( tx, t0, s_phc_coeffs_Hx, s_phc_coeffs_Ht, d_const_phc_coeffs_H, d_const_phc_coeffs_Ht );
          eval_Jacobian_3view_unknownf_pHC< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
          eval_H_3view_unknownf_pHC< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Hx );

          //eval_Jacobian_3view_unknownf_pHC<N, coefsCount>(tx, s_track, r_cgesvA, d_Hx_const_mat_s, d_Hx_const_mat_X1, d_Hx_const_mat_X2, d_Hx_const_mat_X3, d_Hx_const_mat_X4, d_Hx_const_mat_Y, s_cdt);
          //eval_H_3view_unknownf_pHC<coefsCount>(tx, s_track, r_cgesvB, d_Ht_const_mat_s, d_Ht_const_mat_X, d_Ht_const_mat_Y, s_cdt);
    ...
    d_cgesvB[tx] = (t0 >= 1.0 || (1.0-t0 < 0.001)) ? MAGMA_C_MAKE(1.0, hc_step) : MAGMA_C_MAKE(t0, delta_t);
  ...
  kernel_HC_Solver_3view_unknownf_pHC(
  ...
  )
  {
    ...
    //shmem += Hx_idx_size * sizeof(int);
    //shmem += Ht_idx_size * sizeof(int);
    //shmem += phc_coeffs_H_size * sizeof(magmaFloatComplex);   // -- phc coefficients of function t in H --
    //shmem += phc_coeffs_Ht_size * sizeof(magmaFloatComplex);  // -- phc coefficients of function t in Ht --
    ...
    void *kernel_args[] = {&ldda, &d_startSols_array, &d_Track_array, /*&d_startCoefs_array, &d_targetCoefs_array,*/ &d_cgesvA_array, &d_cgesvB_array,
                           &d_Hx_idx_array, &d_Ht_idx_array, &d_phc_coeffs_H, &d_phc_coeffs_Ht};
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 18*12*6=1296 --
    // -- Ht_max_terms*Ht_max_parts = 14*7 = 98 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_3view_unknownf_pHC< 18, 153, 130, 5, 10, 12, 6, 14, 7 >, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_3view_unknownf_pHC is not successful!\n");
    }
