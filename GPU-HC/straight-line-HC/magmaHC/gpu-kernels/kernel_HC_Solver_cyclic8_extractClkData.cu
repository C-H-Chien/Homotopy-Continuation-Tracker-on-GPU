#ifndef kernel_HC_Solver_cyclic8_cu
#define kernel_HC_Solver_cyclic8_cu
// ============================================================================
// homotopy continuation solver for cyclic8 problem
//
// Modifications
//    Chien  21-12-24:   cyclic8 benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-eval/dev-eval-indxing-cyclic8.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_cyclic8_extractClkData(
    magma_int_t ldda,
...
*/
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        long long start=clock64(); // -- start of the clock --
        
        // -- get HxHt for k1 --
        eval_cdt_cyclic8<N, coefsCount>( tx, t0, s_cdt, s_startCoefs, s_targetCoefs );
        eval_Jacobian_cyclic8_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
        eval_Ht_cyclic8_cached<N, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>( tx, s_track, r_cgesvB, d_const_vec_cd, d_Ht_idx);
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          eval_Jacobian_cyclic8_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>(tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
          eval_H_cyclic8_cached<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>(tx, s_track, r_cgesvB, s_cdt, d_Ht_idx);
  ...
  kernel_HC_Solver_cyclic8_extractClkData(
  ...
  )
  {
    ...
    void *kernel_args[] = {&ldda, &d_startSols_array, &d_Track_array, &d_startCoefs_array, &d_targetCoefs_array, &d_cgesvA_array, &d_cgesvB_array,
                           &cm->d_const_cd, &d_Hx_idx_array, &d_Ht_idx_array, &clocks};
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 8*7*9=504 --
    // -- Ht_max_terms*Ht_max_parts = 8*10 = 80 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_cyclic8_extractClkData< 8, 2, 24, 5, 10, 504, 80, 7, 9, 8, 10>, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_cyclic8 is not successful!\n");
    }
