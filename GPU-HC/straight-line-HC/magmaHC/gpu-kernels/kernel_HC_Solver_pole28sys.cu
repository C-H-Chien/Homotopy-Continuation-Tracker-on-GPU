#ifndef kernel_HC_Solver_pole28sys_cu
#define kernel_HC_Solver_pole28sys_cu
// ============================================================================
// homotopy continuation solver for pole28sys problem
//
// Modifications
//    Chien  22-01-24:   pole28sys benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-eval/dev-eval-indxing-pole28sys.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_pole28sys(
    magma_int_t ldda,
    ...
    )
  {
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_cdt_pole28sys<N, coefsCount>( tx, t0, s_cdt, s_startCoefs, s_targetCoefs );
        eval_Jacobian_pole28sys_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
        eval_Ht_pole28sys_cached<N, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>( tx, s_track, r_cgesvB, d_const_vec_cd, d_Ht_idx);
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          eval_Jacobian_pole28sys_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>(tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
          eval_H_pole28sys_cached<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>(tx, s_track, r_cgesvB, s_cdt, d_Ht_idx);
    ...
    d_cgesvB[tx] = (t0 >= 1.0 || (1.0-t0 < 0.00001)) ? MAGMA_C_ONE : MAGMA_C_ZERO;
  ...
  )
  {
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 16*8*3=384 --
    // -- Ht_max_terms*Ht_max_parts = 73*4 = 292 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_pole28sys< 16, 1162, 278, 5, 10, 384, 292, 8, 3, 73, 4>, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_pole28sys is not successful!\n");
    }
