#ifndef kernel_HC_Solver_game7two_cu
#define kernel_HC_Solver_game7two_cu
// ============================================================================
// homotopy continuation solver for game7two problem
//
// Modifications
//    Chien  21-12-24:   game7two benchmark problem; originally created
//
// ============================================================================
#include <stdio.h>
#include <stdlib.h>
...
// -- device function --
#include "../gpu-idx-eval/dev-eval-indxing-game7two.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_game7two(
    magma_int_t ldda,
    ...
    )
  {
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // -- get HxHt for k1 --
        eval_cdt_game7two<N, coefsCount>( tx, t0, s_cdt, s_startCoefs, s_targetCoefs );
        eval_Jacobian_game7two_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
        eval_Ht_game7two_cached<N, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>( tx, s_track, r_cgesvB, d_const_vec_cd, d_Ht_idx);
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          ...
          eval_Jacobian_game7two_cached<N, coefsCount, size_Hx, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>(tx, s_track, r_cgesvA, s_cdt, d_Hx_idx);
          eval_H_game7two_cached<N, coefsCount, size_Ht, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts>(tx, s_track, r_cgesvB, s_cdt, d_Ht_idx);
  ...
  kernel_HC_Solver_game7two_extractClkData(
  ...
  )
  {
    ...
    // -- N*Hx_max_terms*Hx_max_parts = 7*32*7=1568 --
    // -- Ht_max_terms*Ht_max_parts = 64*8 = 512 --
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_game7two< 7, 448, 68, 5, 10, 1568, 512, 32, 7, 64, 8>, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_game7two is not successful!\n");
    }
