#ifndef cpu_hc_solver_alea6_cpp
#define cpu_hc_solver_alea6_cpp
// =======================================================================
// Solve homotopy continuation on cpu for the alea6 problem
//
// Modifications
...
#include "../cpu-eval-HxHt/cpu-eval-HxHt-alea6.h"
#include "../cpu-eval-HxH/cpu-eval-HxH-alea6.h"
...
namespace magmaHCWrapper {
    ...
    magma_queue_t my_queue, int max_steps, std::ofstream &tracks_success_file)
  {
    ...
    const int correction_iterations = 10;
    ...
    for (magma_int_t s=0; s < batchCount; s++) {
      ...
      for(int step = 0; step <= max_steps; step++) {
        if (t0 < 1.0 && (1.0-t0 > 0.0000001)) {
          ...
          // ===================================================================
          // -- get k1 HxHt --
          cpu_eval_HxHt_alea6(s, t0, N, h_Track_cpu + s * N, C0, C1, C2, h_startCoefs_p + s * coefsCount, h_targetCoefs_p + s * coefsCount, h_cgesvA + s * N * N, h_cgesvB + s * N );
          ...
          for(int c = 0; c < correction_iterations; c++) {
            ...
            cpu_eval_HxH_alea6( s, t0, N, h_Track_cpu + s * N, C0, C1, C2, h_startCoefs_p + s * coefsCount, h_targetCoefs_p + s * coefsCount, h_cgesvA + s * N * N, h_cgesvB + s * N );
          ...
          track_success = (t0 > 1.0 || (1.0-t0 < 0.000001));
