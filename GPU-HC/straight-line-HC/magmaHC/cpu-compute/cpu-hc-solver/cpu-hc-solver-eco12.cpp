#ifndef cpu_hc_solver_eco12_cpp
#define cpu_hc_solver_eco12_cpp
// =======================================================================
// Solve homotopy continuation on cpu for the eco12 problem
//
// Modifications
...
#include "../cpu-eval-HxHt/cpu-eval-HxHt-eco12.h"
#include "../cpu-eval-HxH/cpu-eval-HxH-eco12.h"
...
namespace magmaHCWrapper {
    ...
    magma_queue_t my_queue, int max_steps, std::ofstream &tracks_success_file)
  {
    ...
    const magmaFloatComplex C2 = MAGMA_C_ZERO;
    ...
    track_sols_file.open("/users/cchien3/data/cchien3/MyBitBucket/issac-benchmark-hc/converged_HC_tracks.txt");
    ...
    for (magma_int_t s=0; s < batchCount; s++) {
      ...
      for(int step = 0; step <= max_steps; step++) {
        if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
          ...
          // ===================================================================
          // -- get k1 HxHt --
          cpu_eval_HxHt_echo12(s, t0, N, h_Track_cpu + s * N, C0, C1, C2, h_startCoefs_p + s * coefsCount, h_targetCoefs_p + s * coefsCount, h_cgesvA + s * N * N, h_cgesvB + s * N );
          /*if (s == 0 && step == 0) {
            magma_cprint(N, N, h_cgesvA + s * N, N);
          ...
          for(int c = 0; c < correction_iterations; c++) {
            ...
            cpu_eval_HxH_echo12( s, t0, N, h_Track_cpu + s * N, C0, C1, C2, h_startCoefs_p + s * coefsCount, h_targetCoefs_p + s * coefsCount, h_cgesvA + s * N * N, h_cgesvB + s * N );

            /*if (s == 0 && step == 0 && c == 0 && !continue_from_gpu) {
              magma_cprint(N, N, h_cgesvA + s * N, N);
              magma_cprint(N, 1, h_cgesvB + s * N, N);
            }*/
    ...
    /*for (int sol_idx = 0; sol_idx < batchCount; sol_idx++) {
      if (h_Track_Success[sol_idx]) {
        for (int var_idx = 0; var_idx < N; var_idx++) {
          track_sols_file << MAGMA_C_REAL((h_Track_cpu + sol_idx * N)[var_idx]) << "+" << MAGMA_C_IMAG((h_Track_cpu + sol_idx * N)[var_idx]) << "i\n";
        }
        track_sols_file << "\n";
      }
    } */   
