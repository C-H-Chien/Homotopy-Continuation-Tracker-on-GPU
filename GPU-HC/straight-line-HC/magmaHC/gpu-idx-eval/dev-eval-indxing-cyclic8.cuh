#ifndef dev_eval_indxing_cyclic8_cuh_
#define dev_eval_indxing_cyclic8_cuh_
// ============================================================================
...
namespace magmaHCWrapper {
      ...
      for(int i = 0; i < N; i++) {
        ...
        for(int j = 0; j < max_terms; j++) {
          r_cgesvA[i] += r_Hx_idx[j*max_parts + i*max_terms*max_parts] 
                       ...
                       * s_track[ r_Hx_idx[j*max_parts + 3 + i*max_terms*max_parts] ]
                       * s_track[ r_Hx_idx[j*max_parts + 4 + i*max_terms*max_parts] ]
                       * s_track[ r_Hx_idx[j*max_parts + 5 + i*max_terms*max_parts] ]
                       * s_track[ r_Hx_idx[j*max_parts + 6 + i*max_terms*max_parts] ]
                       * s_track[ r_Hx_idx[j*max_parts + 7 + i*max_terms*max_parts] ]
                       * s_cdt[ r_Hx_idx[j*max_parts + 8 + i*max_terms*max_parts] ];
        }
      }
    }
        ...
        const magmaFloatComplex* __restrict__ d_const_cd, magma_int_t r_Ht_idx[size_Ht])
    {
      ...
      for (int i = 0; i < max_terms; i++) {
        r_cgesvB += r_Ht_idx[i*max_parts] 
                  * s_track[ r_Ht_idx[i*max_parts+1] ] 
                  * s_track[ r_Ht_idx[i*max_parts+2] ] 
                  * s_track[ r_Ht_idx[i*max_parts+3] ]
                  * s_track[ r_Ht_idx[i*max_parts+4] ] 
                  * s_track[ r_Ht_idx[i*max_parts+5] ] 
                  * s_track[ r_Ht_idx[i*max_parts+6] ]
                  * s_track[ r_Ht_idx[i*max_parts+7] ]
                  * s_track[ r_Ht_idx[i*max_parts+8] ] 
                  * d_const_cd[ r_Ht_idx[i*max_parts+9] ];
      }
    }
      ...
      for(int i = 0; i < N; i++) {
        ...
        for(int j = 0; j < max_terms; j++) {
          r_cgesvA[i] += d_Hx_idx[j*max_parts + i*max_terms_parts + tx*N_max_terms_parts] 
                       ...
                       * s_cdt[ d_Hx_idx[j*max_parts + 8 + i*max_terms_parts + tx*N_max_terms_parts] ];
        }
      }
    }
      ...
      magmaFloatComplex s_cdt[coefsCount], const int* __restrict__ d_Ht_idx)
    {
      ...
      for (int i = 0; i < max_terms; i++) {
        r_cgesvB += d_Ht_idx[i*max_parts + tx*max_terms_parts] 
                  ...
                  * s_track[ d_Ht_idx[i*max_parts + 7 + tx*max_terms_parts] ]
                  * s_track[ d_Ht_idx[i*max_parts + 8 + tx*max_terms_parts] ] 
                  * s_cdt[ d_Ht_idx[i*max_parts + 9 + tx*max_terms_parts] ];
      }
    }
}
