#ifndef DEV_EVAL_INDXING_DUAL_RECIEVER_TDOA_5PT_CUH
#define DEV_EVAL_INDXING_DUAL_RECIEVER_TDOA_5PT_CUH
// =================================================================================================
    ...
    const magmaFloatComplex __restrict__ *d_phc_coeffs_Ht )
{
  ...
  for (int i = 0; i < Full_Parallel_Offset; i++) {
    s_phc_coeffs_Hx[ tx + i*Num_Of_Vars ] = d_phc_coeffs_Hx[ tx*Max_Order_of_t_Plus_One + i*Num_Of_Vars*Max_Order_of_t_Plus_One ] 
                                          ...
                                          + (((d_phc_coeffs_Hx[ tx*Max_Order_of_t_Plus_One + 3 + i*Num_Of_Vars*Max_Order_of_t_Plus_One ] * t) * t) * t) * t;
    s_phc_coeffs_Ht[ tx + i*Num_Of_Vars ] = d_phc_coeffs_Ht[ tx*Max_Order_of_t + i*Num_Of_Vars*Max_Order_of_t ] 
                                          ...
                                          + ((d_phc_coeffs_Ht[ tx*Max_Order_of_t + 2 + i*Num_Of_Vars*Max_Order_of_t ] * t) * t) * t;
  }
  ...
  if (tx < Partial_Parallel_Thread_Offset) {
    s_phc_coeffs_Hx[ tx + Partial_Parallel_Index_Offset ] = \
      ...
      + ((d_phc_coeffs_Hx[ tx*Max_Order_of_t_Plus_One + Partial_Parallel_Index_Offset_Hx + 3 ] * t) * t) * t
      + (((d_phc_coeffs_Hx[ tx*Max_Order_of_t_Plus_One + Partial_Parallel_Index_Offset_Hx + 3 ] * t) * t) * t) * t;
    ...
    s_phc_coeffs_Ht[ tx + Partial_Parallel_Index_Offset ] = \
      ...
      + (d_phc_coeffs_Ht[ tx*Max_Order_of_t + Partial_Parallel_Index_Offset_Ht + 2 ] * t) * t
      + ((d_phc_coeffs_Ht[ tx*Max_Order_of_t + Partial_Parallel_Index_Offset_Ht + 2 ] * t) * t) * t;
  }
}
