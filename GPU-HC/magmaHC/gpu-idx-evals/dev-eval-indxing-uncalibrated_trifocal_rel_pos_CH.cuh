#ifndef DEV_EVAL_INDXING_UNCALIBRATED_TRIFOCAL_REL_POS_CH_CUH
#define DEV_EVAL_INDXING_UNCALIBRATED_TRIFOCAL_REL_POS_CH_CUH
// =================================================================================================
// Device function for evaluating the Jacobians ∂H/∂x, ∂H/∂t, and H
//
// Modifications
//    Chiang-Heng Chien  24-05-29:   Build on top of generalized three-view relative pose problem.
//
// =================================================================================================
#include <cstdio>
#include <iostream>
  ...
  for(int i = 0; i < Num_Of_Vars; i++) {
    ...
    for(int j = 0; j < dHdx_Max_Terms; j++) {
      r_cgesvA[i] += d_Hx_idx[j*dHdx_Max_Parts + i*dHdx_Entry_Offset + tx*dHdx_Row_Offset] 
                    ...
                    * s_track[      d_Hx_idx[j*dHdx_Max_Parts + 5 + i*dHdx_Entry_Offset + tx*dHdx_Row_Offset] ];
    }
  }
}
    ...
    const int* __restrict__ d_Ht_idx, magmaFloatComplex *s_phc_coeffs)
{
  ...
  for (int i = 0; i < dHdt_Max_Terms; i++) {
    r_cgesvB -= d_Ht_idx[i*dHdt_Max_Parts + tx*dHdt_Row_Offset] 
              ...
              * s_track[      d_Ht_idx[i*dHdt_Max_Parts + 6 + tx*dHdt_Row_Offset] ] ;
  }
}
