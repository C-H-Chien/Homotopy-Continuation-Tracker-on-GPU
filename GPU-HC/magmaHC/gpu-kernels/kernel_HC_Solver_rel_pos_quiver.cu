#ifndef KERNEL_HC_solver_REL_POS_QUIVER_CU
#define KERNEL_HC_solver_REL_POS_QUIVER_CU
// =============================================================================================================
// GPU homotopy continuation solver for Relative Pose Estimation using Quivers with unknown focal length
//
// Modifications
//    Chiang-Heng Chien  24-05-29:   Build on top of 5pt_rel_pos_alg_form_quat
//
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-rel_pos_quiver.cuh"
...
)
{
  ...
  if (tx == 0) {
    s_phc_coeffs_Ht[36] = d_phc_coeffs_Ht[36];
  }
...
real_Double_t
kernel_HC_Solver_rel_pos_quiver(
...
)
{
  ...
  const int num_of_tracks             = 20;
  const int num_of_coeffs_from_params = 36;
  const int dHdx_Max_Terms            = 8;
  const int dHdx_Max_Parts            = 4;
  const int dHdt_Max_Terms            = 18;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_rel_pos_quiver \
  ...
  e = cudaLaunchKernel((void*)HC_solver_rel_pos_quiver \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_rel_pos_quiver is not successful!\n");
