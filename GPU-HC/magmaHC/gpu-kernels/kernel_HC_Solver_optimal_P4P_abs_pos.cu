#ifndef KERNEL_HC_solver_OPTIMAL_P4P_ABS_POS_CU
#define KERNEL_HC_solver_OPTIMAL_P4P_ABS_POS_CU
// =============================================================================================================
// GPU homotopy continuation solver for Optimal P4P Absolute Pose Estimation problem
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-optimal_P4P_abs_pos.cuh"
...
real_Double_t
kernel_HC_Solver_optimal_P4P_abs_pos(
...
)
{
  ...
  const int num_of_tracks             = 32;
  const int num_of_coeffs_from_params = 84;
  const int dHdx_Max_Terms            = 6;
  const int dHdx_Max_Parts            = 3;
  const int dHdt_Max_Terms            = 21;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_optimal_P4P_abs_pos \
  ...
  e = cudaLaunchKernel((void*)HC_solver_optimal_P4P_abs_pos \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_optimal_P4P_abs_pos is not successful!\n");
