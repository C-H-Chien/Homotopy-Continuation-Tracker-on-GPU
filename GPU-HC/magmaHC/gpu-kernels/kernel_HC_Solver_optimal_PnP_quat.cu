#ifndef KERNEL_HC_solver_OPTIMAL_PNP_QUAT_CU
#define KERNEL_HC_solver_OPTIMAL_PNP_QUAT_CU
// =============================================================================================================
// GPU homotopy continuation solver for Optimal PnP (Quaternion form)
//
// Modifications
//    Chiang-Heng Chien  24-05-26:   Build on top of 5pt_rel_pos_alg_form_quat
//
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-optimal_PnP_quat.cuh"
...
)
{
  ...
  for (int i = 0; i < 26; i++) {
    s_phc_coeffs_Ht[tx + i*Num_Of_Vars] = d_phc_coeffs_Ht[tx + i*Num_Of_Vars];
  }
  if (tx == 0) {
    s_phc_coeffs_Ht[105] = d_phc_coeffs_Ht[105];
  }
...
real_Double_t
kernel_HC_Solver_optimal_PnP_quat(
...
)
{
  ...
  const int num_of_vars               = 4;
  const int num_of_tracks             = 80;
  const int num_of_coeffs_from_params = 105;
  const int dHdx_Max_Terms            = 20;
  const int dHdx_Max_Parts            = 5;
  const int dHdt_Max_Terms            = 35;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_optimal_PnP_quat \
  ...
  e = cudaLaunchKernel((void*)HC_solver_optimal_PnP_quat \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_optimal_PnP_quat is not successful!\n");
