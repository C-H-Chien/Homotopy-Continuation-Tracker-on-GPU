#ifndef KERNEL_HC_solver_4VIEW_TRIANGULATION_CU
#define KERNEL_HC_solver_4VIEW_TRIANGULATION_CU
// =============================================================================================================
// GPU homotopy continuation solver for 4-view triangulation
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-4view_triangulation.cuh"
...
)
{
  ...
  if (tx < 3) {
    s_phc_coeffs_Ht[tx + 33] = d_phc_coeffs_Ht[tx + 33];
  }
...
real_Double_t
kernel_HC_Solver_4view_triangulation(
...
)
{
  ...
  const int num_of_tracks             = 142;
  const int num_of_coeffs_from_params = 35;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_4view_triangulation \
  ...
  e = cudaLaunchKernel((void*)HC_solver_4view_triangulation \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_4view_triangulation is not successful!\n");
