#ifndef KERNEL_HC_solver_P3P_10x10_CU
#define KERNEL_HC_solver_P3P_10x10_CU
// =============================================================================================================
// GPU homotopy continuation solver for P3P absolute pose estimation problem (native form, 10x10 formulation)
//
// Modifications
//    Chiang-Heng Chien  25-01-19:   Build on top of the P3P kernel code
//
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-P3P_10x10.cuh"
...
)
{
  ...
  const int num_of_tracks             = 15;
  const int num_of_coeffs_from_params = 15;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_P3P_10x10 \
  ...
  e = cudaLaunchKernel((void*)HC_solver_P3P_10x10 \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_P3P_10x10 is not successful!\n");
