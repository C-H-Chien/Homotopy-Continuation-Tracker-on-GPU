#ifndef KERNEL_HC_solver_DUAL_RECIEVER_TDOA_5PT_CU
#define KERNEL_HC_solver_DUAL_RECIEVER_TDOA_5PT_CU
// =============================================================================================================
// GPU homotopy continuation solver for dual-reciever TDOA 5-points
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-dual_reciever_TDOA_5pt.cuh"
...
__global__ void
HC_solver_dual_reciever_TDOA_5pt(
...
real_Double_t
kernel_HC_Solver_dual_reciever_TDOA_5pt(
...
)
{
  ...
  const int num_of_tracks             = 24;
  const int num_of_coeffs_from_params = 85;
  const int dHdx_Max_Terms            = 15;
  const int dHdx_Max_Parts            = 5;
  const int dHdt_Max_Terms            = 33;
  const int dHdt_Max_Parts            = 6;
  const int max_order_of_t            = 4;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_dual_reciever_TDOA_5pt \
  ...
  e = cudaLaunchKernel((void*)HC_solver_dual_reciever_TDOA_5pt \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_dual_reciever_TDOA_5pt is not successful!\n");
