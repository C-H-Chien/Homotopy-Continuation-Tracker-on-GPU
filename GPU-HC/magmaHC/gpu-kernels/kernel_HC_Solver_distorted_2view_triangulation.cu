#ifndef KERNEL_HC_solver_DISTORED_2VIEW_TRIANGULATION_CU
#define KERNEL_HC_solver_DISTORED_2VIEW_TRIANGULATION_CU
// =============================================================================================================
// GPU homotopy continuation solver for Distorted 2-View Triangulation problem
//
// Modifications
//    Chiang-Heng Chien  24-05-27:   Build on top of 5pt_rel_pos_alg_form_quat
//
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-distorted_2view_triangulation.cuh"
...
__global__ void
HC_solver_distorted_2view_triangulation(
...
real_Double_t
kernel_HC_Solver_distorted_2view_triangulation(
...
)
{
  ...
  const int num_of_vars               = 5;
  const int num_of_tracks             = 28;
  const int num_of_coeffs_from_params = 24;
  const int dHdx_Max_Terms            = 10;
  const int dHdx_Max_Parts            = 5;
  const int dHdt_Max_Terms            = 25;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_distorted_2view_triangulation \
  ...
  e = cudaLaunchKernel((void*)HC_solver_distorted_2view_triangulation \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_distorted_2view_triangulation is not successful!\n");
