#ifndef KERNEL_HC_solver_P3P_CU
#define KERNEL_HC_solver_P3P_CU
// =============================================================================================================
// GPU homotopy continuation solver for P3P absolute pose estimation problem
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-P3P.cuh"
...
)
{
  ...
  const int num_of_coeffs_from_params = 9;
  const int dHdx_Max_Terms            = 2;
  const int dHdx_Max_Parts            = 3;
  const int dHdt_Max_Terms            = 4;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_P3P \
  ...
  e = cudaLaunchKernel((void*)HC_solver_P3P \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_P3P is not successful!\n");
