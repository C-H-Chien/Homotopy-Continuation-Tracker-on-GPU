#ifndef KERNEL_HC_solver_PNP_UNKN_PRINCIPAL_PT_CU
#define KERNEL_HC_solver_PNP_UNKN_PRINCIPAL_PT_CU
// =============================================================================================================
// GPU homotopy continuation solver for PnP Problem with unknown principal point
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-PnP_unkn_principal_pt.cuh"
...
__global__ void
HC_solver_PnP_unkn_principal_pt(
...
real_Double_t
kernel_HC_Solver_PnP_unkn_principal_pt(
...
)
{
  ...
  const int num_of_coeffs_from_params = 28;
  const int dHdx_Max_Terms            = 5;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_PnP_unkn_principal_pt \
  ...
  e = cudaLaunchKernel((void*)HC_solver_PnP_unkn_principal_pt \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_PnP_unkn_principal_pt is not successful!\n");
