#ifndef KERNEL_HC_solver_6PT_RS_ABS_POS_CU
#define KERNEL_HC_solver_6PT_RS_ABS_POS_CU
// =============================================================================================================
// GPU homotopy continuation solver for 6-Point Rolling Shutter Absolute Pose Estimation problem
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-6pt_RS_abs_pos.cuh"
...
real_Double_t
kernel_HC_Solver_6pt_RS_abs_pos(
...
)
{
  ...
  const int num_of_coeffs_from_params = 96;
  const int dHdx_Max_Terms            = 4;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_6pt_RS_abs_pos \
  ...
  e = cudaLaunchKernel((void*)HC_solver_6pt_RS_abs_pos \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_6pt_RS_abs_pos is not successful!\n");
