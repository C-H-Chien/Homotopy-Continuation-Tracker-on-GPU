...
HC_solver_trifocal_2op1p_30x30(
  ...
  const T_index_mat* __restrict__ d_Hx_indices,
  const T_index_mat* __restrict__ d_Ht_indices,
  ...
  for (int step = 0; step <= hc_max_steps; step++) {
    if (t0 < 1.0 && (1.0-t0 > 0.0000001)) {
      ...
      for (int rk_step = 0; rk_step < 4; rk_step++ ) {
        ...
        eval_Jacobian_Hx< T_index_mat, Num_Of_Vars, dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset >( tx, r_cgesvA, s_track, s_startParams, s_targetParams, s_param_homotopy, d_Hx_indices );
        eval_Jacobian_Ht< T_index_mat, Num_Of_Vars, dHdt_Max_Terms, dHdt_Max_Parts >( tx, r_cgesvB, s_track, s_startParams, s_targetParams, s_param_homotopy, d_Ht_indices, s_diffParams );
      ...
      for(int i = 0; i < hc_max_correction_steps; i++) {
        ...
        eval_Jacobian_Hx< T_index_mat, Num_Of_Vars, dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset >( tx, r_cgesvA, s_track, s_startParams, s_targetParams, s_param_homotopy, d_Hx_indices );
        eval_Homotopy< T_index_mat, Num_Of_Vars, dHdt_Max_Terms, dHdt_Max_Parts >( tx, r_cgesvB, s_track, s_startParams, s_targetParams, s_param_homotopy, d_Ht_indices );
...
kernel_HC_Solver_trifocal_2op1p_30x30(
  ...
  magmaFloatComplex*  d_targetParams,
  magmaFloatComplex*  d_diffParams,
  T_index_mat*        d_Hx_indx, 
  T_index_mat*        d_Ht_indx,
  bool*               d_is_GPU_HC_Sol_Converge,
  bool*               d_is_GPU_HC_Sol_Infinity,
  magmaFloatComplex*  d_Debug_Purpose
)
{
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_trifocal_2op1p_30x30 \
                                    <T_index_mat, num_of_vars, num_of_params, dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdt_Max_Terms, dHdt_Max_Parts>, \
