#ifndef KERNEL_HC_solver_6pt_RS_abs_pos_1lin_1LIN_CU
#define KERNEL_HC_solver_6pt_RS_abs_pos_1lin_1LIN_CU
// =============================================================================================================
// GPU homotopy continuation solver for 6-Point Rolling Shutter Absolute Pose Estimation with Linearized Model
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-6pt_RS_abs_pos_1lin.cuh"
...
real_Double_t
kernel_HC_Solver_6pt_RS_abs_pos_1lin(
...
)
{
  ...
  const int num_of_tracks             = 64;
  const int num_of_coeffs_from_params = 72;
  const int dHdx_Max_Terms            = 12;
  const int dHdx_Max_Parts            = 4;
  const int dHdt_Max_Terms            = 36;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_6pt_RS_abs_pos_1lin \
  ...
  e = cudaLaunchKernel((void*)HC_solver_6pt_RS_abs_pos_1lin \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_6pt_RS_abs_pos_1lin is not successful!\n");
