...
namespace magmaHCWrapper {
  ...
  template<int N, int phc_coeffsCount_plus_one, int max_steps, int max_corr_steps, int predSuccessCount, int Hx_max_terms, int Hx_max_parts, int Ht_max_terms, int Ht_max_parts>
  __global__ void
  ...
  )
  {
    ...
    magmaFloatComplex *s_phc_coeffs_Ht = s_phc_coeffs_Hx + phc_coeffsCount_plus_one;
    float* dsx = (float*)(s_phc_coeffs_Ht + phc_coeffsCount_plus_one);
    int* sipiv = (int*)(dsx + N);
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.0000001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_parameter_homotopy<N, N*2, N*3>( tx, t0, s_phc_coeffs_Hx, s_phc_coeffs_Ht, d_const_phc_coeffs_Hx, d_const_phc_coeffs_Ht );
        eval_Jacobian_Hx< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
        eval_Jacobian_Ht< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Ht );

        /*if ( tx == 0 && batchid == 31 ) {
          for (int i = 0; i < N; i++) {
            printf("%f+%f\n", MAGMA_C_REAL(r_cgesvA[i]), MAGMA_C_IMAG(r_cgesvA[i]));
            //printf("%d\n", d_Hx_idx[i]);
          }
        }*/
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          ...
          //eval_parameter_homotopy<N>( tx, t0, s_phc_coeffs_Hx, d_const_phc_coeffs_Hx );
          eval_Jacobian_Hx< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
          eval_Homotopy< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Hx );
  ...
  )
  {
    ...
    shmem += (numOf_phc_coeffs+1) * sizeof(magmaFloatComplex);  // -- s_phc_coeffs_Hx --
    shmem += (numOf_phc_coeffs+1) * sizeof(magmaFloatComplex);  // -- s_phc_coeffs_Ht --
    ...
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_5pt_rel_pos_alg_form_quat< 6, 76, 100, 4, 5, 12, 4, 30, 5 >, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_5pt_rel_pos_alg_form_quat is not successful!\n");
    }
