#ifndef KERNEL_HC_solver_uncalibrated_trifocal_rel_pos_CH_CU
#define KERNEL_HC_solver_uncalibrated_trifocal_rel_pos_CH_CU
// =============================================================================================================
// GPU homotopy continuation solver for uncalibrated (unknown focal length) of trifocal relative pose (4 points)
//
// Modifications
//    Chiang-Heng Chien  24-05-29:   Build on top of generalized three view relative pose
//
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-uncalibrated_trifocal_rel_pos_CH.cuh"
...
__global__ void
HC_solver_uncalibrated_trifocal_rel_pos_CH(
...
real_Double_t
kernel_HC_Solver_uncalibrated_trifocal_rel_pos_CH(
...
)
{
  ...
  const int num_of_vars               = 18;
  const int num_of_tracks             = 5344;
  const int num_of_coeffs_from_params = 88;
  const int dHdx_Max_Terms            = 12;
  const int dHdx_Max_Parts            = 6;
  const int dHdt_Max_Terms            = 14;
  const int dHdt_Max_Parts            = 7;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_uncalibrated_trifocal_rel_pos_CH \
  ...
  e = cudaLaunchKernel((void*)HC_solver_uncalibrated_trifocal_rel_pos_CH \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_uncalibrated_trifocal_rel_pos_CH is not successful!\n");
