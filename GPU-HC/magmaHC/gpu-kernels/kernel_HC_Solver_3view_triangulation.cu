#ifndef KERNEL_HC_solver_3VIEW_TRIANGULATION_CU
#define KERNEL_HC_solver_3VIEW_TRIANGULATION_CU
// =============================================================================================================
// GPU homotopy continuation solver for 3-view triangulation
//
// Modifications
...
//> device functions
#include "../gpu-idx-evals/dev-eval-indxing-3view_triangulation.cuh"
...
)
{
  ...
  if (tx == 0) {
    s_phc_coeffs_Ht[24] = d_phc_coeffs_Ht[24];
  }
...
real_Double_t
kernel_HC_Solver_3view_triangulation(
...
)
{
  ...
  const int num_of_vars               = 8;
  const int num_of_tracks             = 31;
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(HC_solver_3view_triangulation \
  ...
  e = cudaLaunchKernel((void*)HC_solver_3view_triangulation \
                        < num_of_vars, num_of_coeffs_from_params, max_order_of_t, \
                          dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdx_Row_Offset, \
                          dHdt_Max_Terms, dHdt_Max_Parts, dHdt_Row_Offset, \
                          Full_Parallel_Offset, \
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of HC_solver_3view_triangulation is not successful!\n");
