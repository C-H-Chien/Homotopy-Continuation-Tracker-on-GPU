#ifndef kernel_GPUHC_trifocal_2op1p_30x30_PH_OPT_TRUNPATHS_TRUNRANSAC_cu
#define kernel_GPUHC_trifocal_2op1p_30x30_PH_OPT_TRUNPATHS_TRUNRANSAC_cu
// =====================================================================================================
// GPU homotopy continuation solver for the trifocal 2op1p 30x30 problem
//
// Major Modifications
//    Chiang-Heng Chien  24-08-25:   Copied from kernel_GPUHC_trifocal_2op1p_30x30_PH_Opt_TrunPaths.cu 
//                                   as a basis to add early RANSAC abort algorithmic strategy.
//
// =====================================================================================================
#include <stdio.h>
#include <stdlib.h>
...
#include "../dev-trifocal_2op1p-eval.cuh"
...
__global__ void
kernel_GPUHC_trifocal_pose_PH_CodeOpt_TrunPaths_TrunRANSAC(
  const int                 HC_max_steps, 
  const int                 HC_max_correction_steps, 
  const int                 HC_delta_t_incremental_steps,
  const int                 Num_Of_Edgels,            
  const int                 Num_Of_Edgels_Process_Rounds,
  const int                 Num_Of_Remaining_Edgels,  
  const int                 Edgels_Index_Offset,
  ...
  const int* __restrict__   d_unified_dHdx_dHdt_Index,
...
)
{
  ...
  float* s_hypothesis_Rs                  = (float*)(s_RK_Coeffs + 1);
  float* s_intrinsic_matrix               = s_hypothesis_Rs + 18;
  int* s_local_num_of_inlier_supports_21  = (int*)(s_intrinsic_matrix + 4);
  int* s_local_num_of_inlier_supports_31  = (int*)(s_local_num_of_inlier_supports_21 + Num_Of_Vars);

  const int* __restrict__ dHdx_dHdt_Index = d_unified_dHdx_dHdt_Index;
  ...
  s_local_num_of_inlier_supports_21[tx] = 0;
  s_local_num_of_inlier_supports_31[tx] = 0;
  ...
  if (tx == 0) {
    ...
    sipiv[Num_Of_Vars]                = 0;
    s_intrinsic_matrix[0]             = d_Intrinsic_Matrix[0];  //> fx
    s_intrinsic_matrix[1]             = d_Intrinsic_Matrix[4];  //> fy
    s_intrinsic_matrix[2]             = d_Intrinsic_Matrix[3];  //> cx
    s_intrinsic_matrix[3]             = d_Intrinsic_Matrix[5];  //> cy
  }
  magmablas_syncwarp();
    ...
    for (int step = 0; step <= hc_max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.0000001)) {
        ...
        for (int rk_step = 0; rk_step < 4; rk_step++ ) {
          ...
          eval_Jacobian_Hx< Num_Of_Vars, dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset >( tx, r_cgesvA, s_track, s_startParams, s_targetParams, s_param_homotopy, dHdx_dHdt_Index );
          eval_Jacobian_Ht< Num_Of_Vars, dHdt_Max_Terms, dHdt_Max_Parts, dHdx_Index_Matrix_Size >( tx, r_cgesvB, s_track, s_startParams, s_targetParams, s_param_homotopy, dHdx_dHdt_Index, s_diffParams );
        ...
        for(int i = 0; i < hc_max_correction_steps; i++) {
          ...
          eval_Jacobian_Hx< Num_Of_Vars, dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset >( tx, r_cgesvA, s_track, s_startParams, s_targetParams, s_param_homotopy, dHdx_dHdt_Index );
          eval_Homotopy< Num_Of_Vars, dHdt_Max_Terms, dHdt_Max_Parts, dHdx_Index_Matrix_Size >( tx, r_cgesvB, s_track, s_startParams, s_targetParams, s_param_homotopy, dHdx_dHdt_Index );
  ...
  //> Return the solution from device to host
  ...
  //> If the solution is of interest, seek supports from all trifocal correspondences
  //  Note: simply reuse s_RK_Coeffs[0] for an indicator of passing RANSAC supports or not
  if (t0 >= 1.0 || 1.0-t0 <= 0.0000001) {

    evaluate_trifocal_2op1p_30x30_sol< Num_Of_Vars > ( \
      tx, batchid, \
      d_Triplet_Edge_Locations, \
      Num_Of_Edgels, Num_Of_Edgels_Process_Rounds, Num_Of_Remaining_Edgels, Edgels_Index_Offset, \
      s_RK_Coeffs, s_track, dsx, s_hypothesis_Rs, s_intrinsic_matrix, \
      s_local_num_of_inlier_supports_21, s_local_num_of_inlier_supports_31, \
      one_half_delta_t, r_sqrt_sols, r_sqrt_corr,
      d_Found_Trifocal_Sols, d_Trifocal_Sols_Batch_Index );
  }
...
real_Double_t
kernel_GPUHC_trifocal_2op1p_30x30_PH_CodeOpt_TrunPaths_TrunRANSAC(
...
)
{
  ...
  //> Numbers for early aborting RANSAC via evaluations
  int Num_Of_Edgels_Process_Rounds = Num_Of_Triplet_Edgels / num_of_vars;
  int Edgels_Index_Offset          = Num_Of_Edgels_Process_Rounds * num_of_vars;
  int Num_Of_Remaining_Edgels      = Num_Of_Triplet_Edgels - (Num_Of_Edgels_Process_Rounds * num_of_vars);

#if DEBUG_EARLY_RANSAC_ABORT
  printf("Num_Of_Triplet_Edgels        = %d\n", Num_Of_Triplet_Edgels);
  printf("Num_Of_Edgels_Process_Rounds = %d\n", Num_Of_Edgels_Process_Rounds);
  printf("Edgels_Index_Offset          = %d\n", Edgels_Index_Offset);
  printf("Num_Of_Remaining_Edgels      = %d\n", Num_Of_Remaining_Edgels);
#endif
  ...
  shmem += (num_of_vars)     * sizeof(float);                       //> intermediate varaible for cgesv; dsx
  ...
  //> Enabling early abort of RANSAC
  shmem += (18)              * sizeof(float);                       //> s_hypothesis_Rs
  shmem += (4)               * sizeof(float);                       //> s_intrinsic_matrix
  shmem += (num_of_vars)     * sizeof(int);                         //> s_local_num_of_inlier_supports_21
  shmem += (num_of_vars)     * sizeof(int);                         //> s_local_num_of_inlier_supports_31
  ...
  if (shmem <= shmem_max) {
    cudacheck( cudaFuncSetAttribute(kernel_GPUHC_trifocal_pose_PH_CodeOpt_TrunPaths_TrunRANSAC \
  ...
  void *kernel_args[] = { &HC_max_steps, 
                          ...
                          &d_unified_dHdx_dHdt_Index,
                          &d_Triplet_Edge_Locations,
                          &d_is_GPU_HC_Sol_Converge,
                          &d_is_GPU_HC_Sol_Infinity,
                          &d_Debug_Purpose,
                          &d_Intrinsic_Matrix,
                          &d_Found_Trifocal_Sols,
                          &d_Trifocal_Sols_Batch_Index
                        };

  //> launch the GPU kernel
  e = cudaLaunchKernel((void*)kernel_GPUHC_trifocal_pose_PH_CodeOpt_TrunPaths_TrunRANSAC \
                        <num_of_vars, num_of_params, \
                         dHdx_Max_Terms, dHdx_Max_Parts, dHdx_Entry_Offset, dHdt_Max_Terms, dHdt_Max_Parts, \
                         dHdx_Index_Matrix_Size, dHdt_Index_Matrix_Size >, // dHdx_Num_Of_Read_Loops, dHdt_Num_Of_Read_Loops >,
  ...
  if( e != cudaSuccess ) printf("cudaLaunchKernel of kernel_GPUHC_trifocal_pose_PH_CodeOpt_TrunPaths_TrunRANSAC is not successful!\n");
