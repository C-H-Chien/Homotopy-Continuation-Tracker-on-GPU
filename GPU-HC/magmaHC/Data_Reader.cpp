...
bool Data_Reader::Read_Start_Sols(cuFloatComplex* &h_Start_Sols, cuFloatComplex* &h_Homotopy_Sols) {
    ...
    if (!File_Start_Sols) {
        ...
        while (File_Start_Sols >> s_real >> s_imag) {
            (h_Start_Sols + i * (NUM_OF_VARS+1))[d]     = make_cuFloatComplex(s_real, s_imag);
            (h_Homotopy_Sols + i * (NUM_OF_VARS+1))[d]  = make_cuFloatComplex(s_real, s_imag);
        ...
        for(int k = 0; k < NUM_OF_TRACKS; k++) {
            (h_Start_Sols + k * (NUM_OF_VARS+1))[NUM_OF_VARS]    = make_cuFloatComplex(1.0, 0.0);
            (h_Homotopy_Sols + k * (NUM_OF_VARS+1))[NUM_OF_VARS] = make_cuFloatComplex(1.0, 0.0);
        }
        return true;
    }
}

bool Data_Reader::Read_Target_Params(cuFloatComplex* &h_Target_Params) {
    int d = 0;
    ...
    if (!File_Target_Params) {
        ...
        while (File_Target_Params >> s_real >> s_imag) {
            h_Target_Params[d] = make_cuFloatComplex(s_real, s_imag);
            d++;
        }
        return true;
    }
...
bool Data_Reader::Read_Start_Params(cuFloatComplex* &h_Start_Params) {
    int d = 0;
    ...
    if (!File_Start_Params) {
        ...
        while (File_Start_Params >> s_real >> s_imag) {
            h_Start_Params[d] = make_cuFloatComplex(s_real, s_imag);
            d++;
        }
        return true;
    }
