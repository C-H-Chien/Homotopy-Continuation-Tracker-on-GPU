#ifndef kernel_HC_Solver_3pt_rel_pose_w_homo_constraint_cu
#define kernel_HC_Solver_3pt_rel_pose_w_homo_constraint_cu
// ============================================================================
// GPU homotopy continuation solver for 3-point relative pose estimation with 
...
// -- device function --
#include "../gpu-idx-evals/dev-eval-indxing-3pt_rel_pose_w_homo_constraint.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_3pt_rel_pose_w_homo_constraint(
    magma_int_t ldda,
  ...
  )
  {
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        // ===================================================================
        // -- get HxHt for k1 --
        eval_parameter_homotopy<N, N*3, N*2>( tx, t0, s_phc_coeffs_Hx, s_phc_coeffs_Ht, d_const_phc_coeffs_Hx, d_const_phc_coeffs_Ht );
        eval_Jacobian_3pt_rel_pose_w_homo_constraint< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
        eval_Ht_3pt_rel_pose_w_homo_constraint< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Ht );
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          ...
          eval_parameter_homotopy<N, N*3, N*2>( tx, t0, s_phc_coeffs_Hx, s_phc_coeffs_Ht, d_const_phc_coeffs_Hx, d_const_phc_coeffs_Ht );
          eval_Jacobian_3pt_rel_pose_w_homo_constraint< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
          eval_H_3pt_rel_pose_w_homo_constraint< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Hx );
  ...
  kernel_HC_Solver_3pt_rel_pose_w_homo_constraint(
  ...
  )
  {
    ...
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_3pt_rel_pose_w_homo_constraint< 8, 44, 42, 5, 5, 6, 4, 9, 5 >, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_3pt_rel_pose_w_homo_constraint is not successful!\n");
    }
