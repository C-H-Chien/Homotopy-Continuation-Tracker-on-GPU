#ifndef kernel_HC_Solver_refractive_p6p_cu
#define kernel_HC_Solver_refractive_p6p_cu
// ============================================================================
// GPU homotopy continuation solver for refractive p6p problem
//
// Modifications
...
// -- device function --
#include "../gpu-idx-evals/dev-eval-indxing-refractive_p6p.cuh"
...
namespace magmaHCWrapper {
  ...
  __global__ void
  homotopy_continuation_solver_refractive_p6p(
    magma_int_t ldda,
  ...
  )
  {
    ...
    for (int step = 0; step <= max_steps; step++) {
      if (t0 < 1.0 && (1.0-t0 > 0.00001)) {
        ...
        eval_Jacobian_refractive_p6p< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
        eval_Ht_refractive_p6p< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Ht );
        ...
        for(int i = 0; i < max_corr_steps; i++) {
          ...
          eval_parameter_homotopy<N, N*3, N*2>( tx, t0, s_phc_coeffs_Hx, s_phc_coeffs_Ht, d_const_phc_coeffs_Hx, d_const_phc_coeffs_Ht );
          eval_Jacobian_refractive_p6p< N, Hx_max_terms, Hx_max_parts, Hx_max_terms*Hx_max_parts, N*Hx_max_terms*Hx_max_parts>( tx, s_track, r_cgesvA, d_Hx_idx, s_phc_coeffs_Hx );
          eval_H_refractive_p6p< N, Ht_max_terms, Ht_max_parts, Ht_max_terms*Ht_max_parts >( tx, s_track, r_cgesvB, d_Ht_idx, s_phc_coeffs_Hx );
  ...
  kernel_HC_Solver_refractive_p6p(
  ...
  )
  {
    ...
    e = cudaLaunchKernel((void*)homotopy_continuation_solver_refractive_p6p< 6, 108, 62, 5, 10, 10, 5, 26, 6 >, grid, threads, kernel_args, shmem, my_queue->cuda_stream());

    gpu_time = magma_sync_wtime( my_queue ) - gpu_time;
    if( e != cudaSuccess ) {
        printf("cudaLaunchKernel of homotopy_continuation_solver_refractive_p6p is not successful!\n");
    }
